...
import { api, nativeConfirm, tauri } from "../lib/tauri";
import { useAppStore } from "../store/useAppStore";

/**
 * 底部导航条：作为 .app-shell（flex 纵向布局）的最后一个元素，
 * 占用页面布局空间并固定在窗口最底部。
 * 左侧为服务级操作：停止服务 / 重启服务；右侧为设置入口（打开独立设置窗口：
 * 插件管理、通知管理、主题设置、日志管理、系统环境等）。
  ...
  return (
    <footer className="bottombar">
      ...
      <div className="bottombar-right">
        {/* 设置入口：打开独立设置窗口（已开则聚焦，不再占用主窗口内容区）；
            开窗失败（如浏览器预览）回退本窗口内设置壳 */}
        <button
          type="button"
          className="icon-btn"
          title="设置"
          aria-label="设置"
          onClick={() => api.openSettings().catch(() => navigate("/settings"))}
        >
          <SettingOutlined />
        </button>
      </div>
