...
/**
 * 语音合成工具（独立窗口 /tts-studio）：上半是长文本合成工作台——输入大段文字，
 * Rust 侧按句分段（≤120 字/段）逐段合成（每段独立吃缓存）后拼接导出 WAV，
 * 支持播放预览 / 另存为 / 打开所在文件夹；下半是语音配置面板（环境自检、
 * 依赖安装、试听、停止服务）。页面挂在主前端 bundle 的 hash 路由下，
 * 主题、zustand 配置与主窗口共享。
 */
  ...
  return (
    <>
      ...
      <div className="settings-body">
        ...
        <div className="settings-card">
          <div className="settings-card-title">语音配置</div>
          <VoiceConfigPanel />
        </div>
      </div>
    </>
