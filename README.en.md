...
- **pnpm compatible** — works with the pnpm 10 / 11 global layouts (shims in `PNPM_HOME`). Even if `pnpm setup` was never run, it injects the bin dir into the session PATH and persists it to the user PATH, so the service starts immediately and stays installed across launches. **No pnpm major-version gate**: whatever you have installed is what gets used (11 included) — the start chain never force-downgrades or reinstalls your pnpm, and this app **never escalates privileges** (it won't run sudo, and won't tell you to edit system-owned directories).
