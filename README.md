...
| 页面 | 说明 |
| --- | --- |
| `/` 启动页 | 居中 logo + 主按钮。已有服务运行时显示 **打开应用**，否则显示 **启动应用**。 |
| `/terminal` 终端页 | 流式输出全局安装（`pnpm add -g @deepseek-ai/dsh@latest`）与 `dsh web` 启动日志；服务就绪后自动进入预览页。 |
| `/preview` 预览页 | iframe 内嵌本地服务，支持刷新 / 复制地址 / 浏览器打开，并带断开重连提示。 |
