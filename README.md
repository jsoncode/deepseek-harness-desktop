...
- **pnpm 兼容** — 兼容 pnpm 10 / 11 的全局布局（shims 位于 `PNPM_HOME`）：未运行过 `pnpm setup` 的机器也会自动完成临时 PATH 注入与用户级 PATH 持久化，安装后立即启动，下次打开不再重装。**不按 pnpm 主版本设卡**：你装的是哪个版本就用哪个（含 11），启动链不会强制降级或重装你的 pnpm——本应用也**从不提权**（不会用 sudo 装东西，也不会引导你去改系统级目录）。
